...
class EvalRunner:
    ...
    def _calculate_response_score(self, response: ModelResponse) -> float:
        # Calculate a normalized score (0-100) based on various metrics
        clarity_score = self.evaluator._evaluate_clarity(response.response)
        reasoning_score = self.evaluator._evaluate_reasoning_quality(response.response)
        return (clarity_score + reasoning_score) / 2
